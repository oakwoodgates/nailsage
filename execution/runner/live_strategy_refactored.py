"""Refactored live strategy orchestrator with improved testability.

This is the refactored version of LiveStrategy that uses smaller, testable components:
- CandleCloseDetector: Handles candle close detection
- TradeExecutionPipeline: Orchestrates trading workflow

The refactored design makes it easier to:
...
class LiveStrategyRefactored:
    """
    Refactored live strategy orchestrator with improved architecture.

    Key improvements over original LiveStrategy:
    ...
    def __init__(
        ...
        """
        Initialize refactored live strategy.
        ...
        logger.info(
            f"Initialized LiveStrategyRefactored: {config.strategy_name}",
            extra={
    ...
    async def _update_positions_pnl(self, price: float) -> None:
        ...
        await asyncio.to_thread(
            self.pipeline.position_tracker.update_all_positions_pnl,
            price
        )
